...
namespace Core.UI
{
    public class WorldSpaceObjectCanvas : MonoBehaviour
    {
        ...
        {
            if (enablePoints != !Player.Player.inst.land.isLanded)
            {
                foreach (var wsp in spaceObjects)
                {
                    wsp.CanvasPoint.gameObject.SetActive(!Player.Player.inst.land.isLanded);
                }
                enablePoints = !Player.Player.inst.land.isLanded;
            }
        ...
        public void UpdatePoints()
        {
            ...
            if (SetActiveObjects())
            {
                var target = Player.Player.inst.GetTarget();
                if (target != null && target.TryGetComponent(out ContactObject contact))
                {
                    ...
                    if (angle < 60)
                    {
                        ...
                        contactText.text = target.transform.name + $"[{Vector3.Distance(position, Player.Player.inst.transform.position).ToString("F5")}]";
                    }
                    else
                    {
                ...
                foreach (var wsp in spaceObjects)
                {
                    if (wsp.Obj.isVisible)
                    {
                        ...
                        if (Player.Player.inst.GetTarget() == wsp.Obj)
                        {
                            wsp.CanvasPoint.SetText(wsp.Obj.transform.name + $" [{Vector3.Distance(wsp.Obj.transform.position, Player.Player.inst.transform.position).ToString("F5")}]");
                        }
                        else
                        {
